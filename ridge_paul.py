...
import numpy as np
import pandas as pn
from response_surfaces import PolynomialApproximation
...
def RidgeAlternating(X, f, U0, degree=1, maxiter=100, tol=1e-10, history = False, disp = False, gtol = 1e-6):
	if len(f.shape) == 1:
		f = f.reshape(-1,1)
	...
	while i < maxiter and res > tol:
		...
		solver = SteepestDescent(logverbosity=0, mingradnorm = gtol)
